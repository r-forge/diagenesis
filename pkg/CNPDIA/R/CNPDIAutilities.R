...
CreateMeanPars <- function(Parms, name, forc, times) {
  ...
  if (is.null(forc$data))
    return(Parms)
  else if (is.null(times) | any(is.na(times)))
...
CheckProfile <- function(prof, Name, interface = TRUE) {  
  ...
  if (is.list (prof)) {
    ...
    if (! "mid" %in%  nms)
      stop (Name, " should be a list containing 'int' and 'mid'")
    if (length(prof$int) != .CNPDIA$N +1)
      stop ("Checking", Name, ": 'int' should be a vector of length ", .CNPDIA$N +1)
    if (length(prof$mid) != .CNPDIA$N )
      stop ("Checking", Name, ": 'mid' should be a vector of length ", .CNPDIA$N )
    return(prof)
  }
  if (interface) {
    if (length(prof) == 1) prof <- rep(prof, .CNPDIA$N+1)
    if (length(prof) != .CNPDIA$N+1)
      stop ("Checking", Name, ": 'should be a vector of length ", .CNPDIA$N +1)
    new <- list(int = prof, mid = 0.5*(prof[-1] + prof[-(.CNPDIA$N+1)]))
  } else {
    if (length(prof) == 1) prof <- rep(prof, .CNPDIA$N)
    if (length(prof) != .CNPDIA$N)
      stop ("Checking", Name, ": 'should be a vector of length ", .CNPDIA$N)
    new <- list(mid = prof, ind = c(prof[1], 0.5*(prof[-1] + prof[-(.CNPDIA$N)]),prof[.CNPDIA$N]))
  }  
  return (new)
}  
